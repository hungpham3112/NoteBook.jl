...
# ╔═╡ d4e84f5a-faff-4745-8fbd-73ee36ef3343
begin
	using Plots # Plots in Julia
	using PlutoUI # Interactive Feature for Pluto
	using Images, ImageView
	using DataFrames, CSV
end

# ╔═╡ 65175f80-1ec3-11ed-04d7-25b1d19f4730
md"""
# Logistic Regression for dummy
"""

# ╔═╡ 68a46d3c-340f-4012-bc0e-a2da80672ea5
md"""
##### Import library
"""

# ╔═╡ 1aedf27e-0fbe-41f9-a90a-8853d839c4fb
sigmoid(x) = 1 / (1 + ℯ^(-x))

# ╔═╡ d8881465-449d-46c8-b074-8159146ada34
sigmoid_plot = plot(-6:0.5:6, sigmoid,
	xlabel = "x",
	ylabel = "Sigmoid",
	color = :red
)

# ╔═╡ e5b7cc5d-6456-4e1e-808a-6af081828fe6
@bind θ₀ Slider(-1:0.05:1, default=0)

# ╔═╡ 1c203e64-78e7-4205-809c-4e349250498f
@bind θ₁ Slider(-1:0.05:1, default=1)

# ╔═╡ 2a139cc3-3805-4ce7-ac56-4350eb422e31
f(x) = θ₀ + θ₁ * x

# ╔═╡ d4267c3d-ecc8-4123-b7f2-710667ac41bf
sigmoid_theta(x) = 1 / (1 + exp.(-f(x)))

# ╔═╡ 31dc9413-1368-4810-bc23-631e3d28aa7d
sigmoid_theta_plot = plot(-6:0.5:6, sigmoid_theta,
	xlabel = "x",
	ylabel = "Sigmoid",
	color = :cyan,
	legend = false
...
# ╔═╡ 25e5b7c2-00ae-473c-8540-c14ea7162db6
θ₀,θ₁

# ╔═╡ 70b04fac-801e-4bab-a498-a7cf2d926eb5
md"""
### Download the data 
"""

# ╔═╡ 9afcf6d9-a680-457d-9bd9-20ae5407f7f5
df = DataFrame(CSV.File("./data.csv"))

# ╔═╡ 9305344a-d0e3-4ed0-a7b5-ad7c55add1fe
begin
	X = df.feature
	Y_temp = df.class
	[x for x in Y_temp ]
	Y = [y == "absent" ? 0 : 1 for y in Y_temp]
end

# ╔═╡ 030d4a00-e465-4044-88af-be3fdea3bbe8
p_data = plot(X, Y,
...
# ╔═╡ a2679857-2834-4850-933d-b404b9f2744b
begin
	z(x) = θ₀ .+ θ₁ * x
	h(x) = 1 ./ (1 .+ exp.(-z(x)))
end

# ╔═╡ 5fd27c4f-08fc-44fa-ab37-a139f6264ead
begin
...
# ╔═╡ 90e61778-634e-4948-80ab-e69f9503fc2a
function pd_theta_0()
    sum(y_hat - Y)
end

# ╔═╡ a1ce3b2d-a5bf-414f-803f-27c2204b5d33
function pd_theta_1()
    sum((y_hat - Y) .* X)
end

# ╔═╡ 6941fc4d-1401-44b1-9a06-e65362cbd624
begin
	J_history = []
	push!(J_history, J)
	t0_history = []
	t1_history = []
	theta0 = 0.0    # y-intercept (default = 0)
	theta1 = 1.0    # slope (default = 1)
	α = 0.01
	# initialize epochs
	epochs = 0
end

# ╔═╡ a9eeeafd-cf6d-4e5c-aa7d-80e394073109
t0_history

# ╔═╡ f7d32914-bc0d-415f-88e9-6a5f89ec4713
for i in 1:9
		# calculate partial derivatives
		theta_0_temp = pd_theta_0()
		theta_1_temp = pd_theta_1()
		# adjust parameters by the learning rate
		theta0 -= α * theta_0_temp
		theta1 -= α * theta_1_temp
		push!(t0_history, theta0)
		push!(t1_history, theta1)
		# recalculate cost
		y_hat = h(X)
		push!(J_history, J)
		# replot prediction
...
# ╔═╡ 37fc12a1-6658-4a29-97cf-b1f2916d5f7d
J_history

# ╔═╡ ba8940a2-357b-4666-bc2d-2c40fe25a702


# ╔═╡ f87761b2-b3a0-419e-aeeb-12cd820d6d3a


# ╔═╡ d505a365-825b-42aa-b211-f8f84917bd56
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "39d76d3eb8a43f4bff6edb50600b8c93a4f36877"
